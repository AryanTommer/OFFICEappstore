...
from TechKiteOFFICEappstore.core.models import App, Category, Screenshot
...
class AppScaffoldingForm(Form):
    ...
    platform = ChoiceField(choices=lazy(get_versions, list), required=True,
                           label=_('TechKiteOFFICE version'))
        ...
        try:
            ...
            if user.is_superuser:
                ...
                send_mail("New integration submitted", "Please review the "
                                                       ...
                                                       "sure it fits the "
                                                       "guidelines.",
                          settings.TechKiteOFFICE_FROM_EMAIL,
                          settings.TechKiteOFFICE_INTEGRATIONS_APPROVAL_EMAILS)
            p.save()
            if self.data['screenshot']:
                screenshot = Screenshot.objects.create(
                    url=self.cleaned_data['screenshot'],
