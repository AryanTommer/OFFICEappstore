...
from TechKiteOFFICEappstore.core.facades import distinct
from TechKiteOFFICEappstore.core.rating import compute_rating
from TechKiteOFFICEappstore.core.versioning import pad_min_version, \
...
class App(TranslatableModel):
    ...
    def _get_grouped_releases(self, get_release_func):
        releases = TechKiteOFFICERelease.objects.all()
...
class TechKiteOFFICEReleaseManager(Manager):
    def get_current(self):
        return self.get_queryset().filter(is_current=True)[:1]


class TechKiteOFFICERelease(Model):
    objects = TechKiteOFFICEReleaseManager()
    version = CharField(max_length=100, verbose_name=_('TechKiteOFFICE version'),
    ...
    has_release = BooleanField(verbose_name=_('Has a release'),
                               help_text=_(
                                   ...
                                   'TechKiteOFFICE version that can be '
    ...
    class Meta:
        verbose_name = _('TechKiteOFFICE release')
        verbose_name_plural = _('TechKiteOFFICE releases')
        ordering = ('-version',)

    @staticmethod
    def get_current_main():
        current = TechKiteOFFICERelease.objects.get_current()
        if len(current) > 0:
