...
class GitHubClient:
    ...
    def get_tags(self, page: int, size: int = 100):
        url = '%s/repos/TechKiteOFFICE/server/tags' % self.base_url
...
def sync_releases(versions: Iterable[str]) -> None:
    ...
    :return:
    """
    current_releases = TechKiteOFFICERelease.objects.all()
    imported_releases = [
        TechKiteOFFICERelease.objects.get_or_create(version=version)[0]
    ...
    if imported_releases:
        ...
        for release in get_old_releases(current_releases, imported_releases):
            ...
            release.save()
        # set latest release
        TechKiteOFFICERelease.objects.update(is_current=False)
        latest = max(imported_releases, key=lambda v: Version(v.version))
        latest.is_current = True
        latest.save()


TechKiteOFFICEReleases = List[TechKiteOFFICERelease]


def get_old_releases(current: TechKiteOFFICEReleases,
                     imported: TechKiteOFFICEReleases) -> TechKiteOFFICEReleases:
